...
<figure>
  ...
  <figcaption><b>
  Fig. 4: We find new key and value matrices for localized cross-attention layers such that they map original text-embeddings (*e.g.*, text-embeddings of prompts having Van Gogh style) to target text-embeddings (*e.g.*, embeddings coreesponding to similar prompts without Van Gogh).
  </b></figcaption>
</figure>
