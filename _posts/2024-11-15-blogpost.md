...
1. [Mechanistic Interpretability Beyond Language Models](#mechanistic-interpretability-beyond-language-models)
   - [Abstract](#abstract)
...
4. [Mechanistically Understanding Model Components Across Different ViTs](#mechanistically-understanding-model-components-across-different-vits)
...
More specifically, we solve the following optimization problem for key and value matrices of localized crooss attention layers

<figure>
  <img src="/images/locoedit-formula.png" alt="Image description">
</figure>

where X<sub>orig</sub> refers to text-embeddings of prompts with the particualr attribute (*e.g.*, Van Gogh style) and X<sub>target</sub> refers to corresponding text-embedding without the attribute, *e.g.*, X<sub>orig</sub> includes text-embeddings of prompts like "A house in Van Gogh style" and X<sub>target</sub> includes text-embeddings of prompts like "A painting of a house".


<figure>
  <img src="/images/locoedit.png" alt="Image description">
  <figcaption><b>Fig. 4: Editing key and value matrices in localized cross-attention layers can effectively prevent the model from generating artistic style, trademark objects, and outdated facts. </b></figcaption>
</figure>
